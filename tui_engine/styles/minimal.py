...
class MinimalStyle(BaseStyle):
    ...
    def _render_header(self, block: ContentBlock) -> Text:
        if block.level == 1:
            return Text(block.content, style="bold white")
        return Text(block.content, style="italic bright_white")

    def _render_paragraph(self, block: ContentBlock) -> Text:
        return Text(block.content, style="dim white")

    def _render_list(self, block: ContentBlock) -> Text:
        lines = []
        for i, item in enumerate(block.items, 1):
            num = Text(f"  {i}. ", style="grey62")
            body = Text(item, style="dim white")
    ...
    def _render_table(self, block: ContentBlock) -> Table:
        ...
        for header in block.headers:
            table.add_column(header, style="dim white", no_wrap=False)
