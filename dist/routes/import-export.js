...
function deckOwnerFilter(userId) {
    return userId ? eq(decks.userId, userId) : (decks.userId === null);
}
// ── Import schema ──
...
router.post("/decks/:id/import", validateBody(importCardsSchema), async (req, res) => {
    ...
    try {
        ...
        if (skipDuplicates) {
            ...
            cardsToInsert = importCards.filter((c) => !existingFronts.has(c.front.trim().toLowerCase()));
        }
        ...
        const now = new Date();
        const inserted = await db.insert(cards).values(cardsToInsert.map((c) => ({
            deckId,
            front: c.front,
            back: c.back,
...
router.post("/decks/bulk-export", validateBody(bulkExportSchema), async (req, res) => {
...
router.post("/import/parse", validateBody(parseImportSchema), async (req, res) => {
