...
import { db, chatMessages, agentUsage, decks, cards, cardProgress, studySessions, exams, groupStudyRooms } from "../db/index.js";
...
function trackUsage(userId, agentId, tokensUsed, durationMs, success) {
...
function isEnabled(req, setting) {
    return req.user?.[setting] !== false;
}
router.post("/chat", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { message, deckId, mode } = req.body;
...
router.post("/smart-review", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { deckId, count = 30 } = req.body;
    const startTime = Date.now();
    try {
        ...
        for (const card of allCards) {
            ...
            if (progress && progress.total > 0) {
                ...
                if (ratio < 0.5)
                    weakCards.push({ ...card, reason: `Low accuracy (${Math.round(ratio * 100)}%)`, score: ratio });
                if (progress.total >= 3 && ratio < 0.7)
                    atRiskCards.push({ ...card, reason: "Needs reinforcement", score: ratio });
            }
            if (progress) {
                const daysSinceStudy = (Date.now() - progress.lastStudiedAt) / 86400000;
                if (daysSinceStudy > 7)
                    overdueCards.push({ ...card, reason: `Not studied recently`, score: 0.5 });
            }
        }
        const prioritized = [
...
router.post("/deck-doctor", requireAuth, async (req, res) => {
    const userId = getUserId(req);
...
router.post("/deck-doctor/fix", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { cardId, fixType, relatedCardId } = req.body;
    const startTime = Date.now();
    try {
...
router.post("/generate-exam", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { deckIds, questionCount = 50, durationMinutes = 60, title } = req.body;
    const startTime = Date.now();
...
router.post("/mnemonics", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { concept, cardIds, deckId } = req.body;
    const startTime = Date.now();
    try {
        let content = concept || "";
...
router.post("/mnemonics/save", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { cardId, mnemonic } = req.body;
    try {
...
router.get("/coach", requireAuth, async (req, res) => {
    ...
    try {
        ...
        for (const p of progressEntries) {
            ...
            for (const tag of tags) {
                if (!tagPerformance[tag])
                    tagPerformance[tag] = { known: 0, unknown: 0 };
        ...
        if (daysSinceLastSession > 2)
            recommendations.push(`You haven't studied in ${daysSinceLastSession} days. Your streak is at risk!`);
        if (weakTopics.length > 0)
            recommendations.push(`Focus on ${weakTopics.slice(0, 3).join(", ")} — these are your weakest areas.`);
        if (bestHour >= 0)
            recommendations.push(`You study most effectively around ${bestHour > 12 ? bestHour - 12 : bestHour}:00 ${bestHour >= 12 ? "PM" : "AM"}.`);
        if (overallAccuracy < 60)
            recommendations.push("Your overall accuracy is below 60%. Consider reviewing fundamentals before adding new cards.");
        if (sessions.length < 7)
            recommendations.push("Try to study daily — consistency is key for long-term retention.");
        if (strongTopics.length > 0)
            recommendations.push(`Great progress in ${strongTopics.join(", ")}! Keep it up.`);
        const weeklyPlan = [
...
router.post("/image-analyze", requireAuth, uploadImage.single("image"), async (req, res) => {
    const userId = getUserId(req);
    const startTime = Date.now();
    if (!req.file) {
...
router.post("/voice-check", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { cardFront, cardBack, spokenAnswer } = req.body;
    const startTime = Date.now();
...
router.post("/group-study/create", requireAuth, async (req, res) => {
    const userId = getUserId(req);
...
router.post("/group-study/join", requireAuth, async (req, res) => {
    const userId = getUserId(req);
...
router.post("/group-study/question", requireAuth, async (req, res) => {
    const userId = getUserId(req);
    const { roomId } = req.body;
    const startTime = Date.now();
    try {
...
router.get("/group-study/:roomId", requireAuth, async (req, res) => {
    try {
...
router.get("/usage", requireAuth, async (req, res) => {
    try {
        ...
        for (const u of usage) {
            if (!byAgent[u.agentId])
                byAgent[u.agentId] = { calls: 0, tokens: 0, successRate: 0 };
