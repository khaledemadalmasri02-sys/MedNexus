...
router.put("/settings", requireAuth, validateBody(updateSettingsSchema), async (req, res) => {
    try {
...
router.post("/settings/sync", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (existing) {
            ...
            await db.insert(userSettings).values(merged);
        }
        const updated = await db.query.userSettings.findFirst({
            where: eq(userSettings.userId, userId),
        });
