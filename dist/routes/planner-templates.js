...
function templateOwnerFilter(userId) {
...
router.post("/generate", validateBody(generatePlanSchema), async (req, res) => {
    try {
        ...
        const studyDayNames = studyDays.map((d) => dayNames[d]).join(', ');
...
Return ONLY valid JSON, no markdown or explanation.`;
        ...
        try {
            ...
            const dayList = studyDays.sort((a, b) => a - b);
            let sessionIdx = 0;
            for (let week = 0; week < Math.ceil(daysUntilExam / 7); week++) {
                for (const day of dayList) {
                    ...
                    sessionIdx++;
                    if (sessionIdx >= daysUntilExam * hoursPerDay / 2)
                        break;
                }
            }
        }
...
router.post("/batch-create", validateBody(batchCreatePlansSchema), async (req, res) => {
    try {
        const userId = getUserId(req);
...
//# sourceMappingURL=planner-templates.js.map