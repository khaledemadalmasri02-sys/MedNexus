...
import { db, generationLogs, decks } from "../db/index.js";
...
router.get("/stats", async (req, res) => {
    try {
        ...
        const stats = await db.select({
            ...
            successfulGenerations: sql `sum(case when ${generationLogs.success} = 1 then 1 else 0 end)`,
            failedGenerations: sql `sum(case when ${generationLogs.success} = 0 then 1 else 0 end)`,
            totalDuration: sql `sum(${generationLogs.durationMs})`,
            avgDuration: sql `avg(${generationLogs.durationMs})`,
            totalTokens: sql `sum(${generationLogs.promptTokens} + ${generationLogs.completionTokens})`,
        })
            .from(generationLogs)
            .where(whereClause);
        // Get by type
