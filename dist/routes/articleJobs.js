...
router.get("/decks/:deckId/topics", async (req, res) => {
    const deckId = parseInt(req.params.deckId, 10);
    if (isNaN(deckId)) {
        res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Invalid deck ID" } });
        return;
...
// Create a job
router.post("/decks/:deckId/article-jobs", async (req, res) => {
    const deckId = parseInt(req.params.deckId, 10);
...
// Get a single job
router.get("/article-jobs/:id", async (req, res) => {
    try {
...
// Delete a job
router.delete("/article-jobs/:id", async (req, res) => {
    try {
        const job = await db.query.articleJobs.findFirst({ where: eq(articleJobs.id, req.params.id) });
        if (!job) {
            res.status(404).json({ error: { code: "NOT_FOUND", message: "Job not found" } });
...
// SSE stream
router.get("/article-jobs/:id/stream", async (req, res) => {
    const { id } = req.params;
    try {
...
//# sourceMappingURL=articleJobs.js.map