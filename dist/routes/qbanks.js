...
import { createQBankSchema, updateQBankSchema, idParamSchema, } from "./validators.js";
const router = Router();
...
// QBank owner filter
function qbankOwnerFilter(userId) {
...
router.post("/", validateBody(createQBankSchema), async (req, res) => {
...
// Update QBank
router.patch("/:id", validateParams(idParamSchema), validateBody(updateQBankSchema), async (req, res) => {
...
// Update question
router.patch("/:qbankId/questions/:questionId", async (req, res) => {
...
// Delete question
router.delete("/:qbankId/questions/:questionId", async (req, res) => {
    const qbankId = parseInt(req.params.qbankId, 10);
    const questionId = parseInt(req.params.questionId, 10);
    if (isNaN(qbankId) || isNaN(questionId)) {
        res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Invalid IDs" } });
...
router.post("/:id/import", async (req, res) => {
    ...
    try {
        ...
        const importedQuestions = await db.insert(questions).values(deckCards.map(card => ({
            qbankId: id,
            front: card.front,
            back: card.back,
            choices: null,
...
//# sourceMappingURL=qbanks.js.map