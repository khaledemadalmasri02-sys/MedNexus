...
// Log generation
async function logGeneration(userId, type, model, success, errorMessage, promptTokens, completionTokens, durationMs) {
    try {
...
router.post("/", validateBody(generateSchema), async (req, res) => {
    ...
    try {
        let generatedItems;
        let deckKind = deckType === "qbank" ? "qbank" : "deck";
        ...
        const createdCards = await db.insert(cards).values(generatedItems.map((item) => {
            const isQuestion = "choices" in item;
            const question = isQuestion ? item : null;
            const card = !isQuestion ? item : null;
            return {
                deckId: deck.id,
                front: item.front,
                back: item.back,
        ...
        await logGeneration(userId, deckKind, usedOfflineFallback ? "offline-fallback" : model, true, undefined, undefined, undefined, duration);
        res.status(201).json({
            deck,
            cards: createdCards,
        ...
        const duration = Date.now() - startTime;
        await logGeneration(userId, deckType, model, false, err.message, undefined, undefined, duration);
...
router.post("/stream", async (req, res) => {
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        await logGeneration(userId, deckType, usedOfflineFallback ? "offline-fallback" : model, true, undefined, undefined, undefined, duration);
        sendEvent("complete", {
            deck,
            cards: createdCards,
            generationId: deck.id,
