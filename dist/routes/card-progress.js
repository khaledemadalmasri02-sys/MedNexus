...
import { db, cardProgress, cards } from "../db/index.js";
...
router.post("/cards/:id/review", validateBody(reviewCardSchema), async (req, res) => {
    try {
        ...
        const sm2 = sm2Update({ easeFactor: progress.easeFactor, intervalDays: progress.intervalDays, repetitions: progress.repetitions }, quality);
...
router.get("/decks/:id/review-queue", async (req, res) => {
    try {
        ...
        const dueProgress = await db.query.cardProgress.findMany({
            where: and(inArray(cardProgress.cardId, cardIds), lte(cardProgress.nextReviewDate, today), ownerFilter(userId)),
        });
...
router.get("/decks/:id/progress", async (req, res) => {
    try {
        const deckId = parseInt(req.params.id, 10);
        if (isNaN(deckId)) {
            res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Invalid deck ID" } });
...
//# sourceMappingURL=card-progress.js.map