...
function parseDelimited(content, delimiter) {
    ...
    return records
        ...
        correctIndex: row.correctIndex !== undefined
...
router.post("/cards", upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        try {
            ...
            cleanup(file.path);
            res.status(400).json({
                error: { code: "PARSE_ERROR", message: `Failed to parse file: ${parseErr.message}` },
            });
            return;
        }
        cleanup(file.path);
...
router.post("/pdf-cards", async (req, res) => {
    try {
...
router.post("/cards/create", async (req, res) => {
    try {
        const { cards, deckName, deckType, skipDuplicates } = req.body;
