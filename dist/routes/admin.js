...
router.get("/stats", async (_req, res) => {
    try {
        ...
        const [errorCount] = await db.select({ count: sql `count(*)` }).from(errorLogs);
        const [unresolvedCount] = await db.select({ count: sql `count(*)` }).from(errorLogs).where(eq(errorLogs.resolved, false));
        const [feedbackCount] = await db.select({ count: sql `count(*)` }).from(feedback);
        res.json({
...
router.get("/errors", async (req, res) => {
    try {
        ...
        if (model)
            conditions.push(eq(errorLogs.model, model));
        const errors = await db.select().from(errorLogs)
            .where(conditions.length > 0 ? and(...conditions) : undefined)
            .limit(limit)
            .offset(offset)
            .orderBy(desc(errorLogs.createdAt));
        const [count] = await db.select({ count: sql `count(*)` }).from(errorLogs)
            .where(conditions.length > 0 ? and(...conditions) : undefined);
...
router.delete("/errors/resolved", async (_req, res) => {
    try {
...
router.get("/feedback", async (req, res) => {
    try {
        ...
        const [count] = await db.select({ count: sql `count(*)` }).from(feedback).where(conditions);
