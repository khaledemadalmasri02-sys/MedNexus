...
Return ONLY the JSON array, no other text.`;
        const analysisText = await aiService.complete([{ role: "user", content: prompt }], { maxTokens: 4000 });
...
router.post("/cards/:id/improve", async (req, res) => {
...
Return ONLY the JSON, no other text.`;
        const result = await aiService.complete([{ role: "user", content: prompt }], { maxTokens: 1000 });
        try {
...
router.post("/decks/:id/insights", async (req, res) => {
    const deckId = parseInt(req.params.id, 10);
    if (isNaN(deckId)) {
        res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Invalid deck ID" } });
        return;
    ...
    try {
        ...
        const cardsList = sampleCards.map((c, i) => `${i + 1}. Q: ${c.front}\n   A: ${c.back}`).join("\n");
...
Return ONLY the JSON, no other text.`;
        const result = await aiService.complete([{ role: "user", content: prompt }], { maxTokens: 3000 });
        try {
            const jsonMatch = result.match(/\{[\s\S]*\}/);
            if (jsonMatch) {
                const parsed = JSON.parse(jsonMatch[0]);
