...
router.post("/", validateBody(startStudySessionSchema), async (req, res) => {
    try {
...
router.patch("/:id", validateBody(updateStudySessionSchema), async (req, res) => {
    try {
        const userId = getUserId(req);
...
router.post("/:id/end", validateBody(endStudySessionSchema), async (req, res) => {
    try {
        const userId = getUserId(req);
        const sessionId = parseInt(req.params.id, 10);
        if (isNaN(sessionId)) {
...
router.get("/stats", async (req, res) => {
    try {
        ...
        const sessions = await db.query.studySessions.findMany({
            where: and(userId ? eq(studySessions.userId, userId) : isNull(studySessions.userId), gte(studySessions.startedAt, weekStart)),
        });
...
router.get("/history", async (req, res) => {
    try {
        ...
        const [countResult] = await db.select({ count: sql `count(*)` }).from(studySessions).where(userId ? eq(studySessions.userId, userId) : isNull(studySessions.userId));
        res.json({
            sessions,
            pagination: {
...
router.get("/summary", async (req, res) => {
    try {
        ...
        const sessions = await db.query.studySessions.findMany({
            where: and(userId ? eq(studySessions.userId, userId) : isNull(studySessions.userId), gte(studySessions.startedAt, startDate)),
        });
        const totalMinutes = sessions.reduce((s, sess) => s + (sess.durationMinutes || 0), 0);
...
//# sourceMappingURL=study-sessions.js.map