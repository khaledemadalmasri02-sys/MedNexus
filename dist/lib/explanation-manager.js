 ...
 */
async function processChunk(chunk, deckId, progress) {
    ...
    const promises = chunk.map(async (card) => {
        try {
            ...
            progress.completed++;
            logger.info({ deckId, cardId: card.id, progress: `${progress.completed}/${progress.total}` }, "Generated explanations for card");
        }
        catch (err) {
            progress.failed++;
            logger.error({ err, deckId, cardId: card.id }, "Failed to generate explanations for card");
        }
    });
 ...
 */
export async function generateExplanationsForDeck(deckId) {
    ...
    try {
        ...
        const cardsWithoutExplanations = await db.query.cards.findMany({
            where: and(eq(cards.deckId, deckId), isNull(cards.explanationFull)),
        });
        ...
        for (let i = 0; i < cardsWithoutExplanations.length; i += CHUNK_SIZE) {
            chunks.push(cardsWithoutExplanations.slice(i, i + CHUNK_SIZE).map(card => ({
                id: card.id,
                front: card.front,
                back: card.back,
            })));
        ...
        for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
            ...
            logger.info({ deckId, chunk: chunkIndex + 1, totalChunks: chunks.length, cardsInChunk: chunk.length }, "Processing chunk");
 ...
 */
export async function hasCardsWithoutExplanations(deckId) {
 ...
 */
export async function getExplanationStats(deckId) {
...
//# sourceMappingURL=explanation-manager.js.map