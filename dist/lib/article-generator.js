...
async function collectDescendantDeckIds(rootDeckId) {
    const allDecks = await db.query.decks.findMany();
    const result = [rootDeckId];
...
function capCardsText(rows, maxChars) {
    const parts = [];
    let total = 0;
    for (const row of rows) {
        const block = `- ${row.front}: ${row.back}`;
        if (total + block.length + 1 > maxChars)
            break;
        parts.push(block);
...
async function buildOutline(topic, cardsText) {
...
function offlineOutline(topic, cardsText) {
...
async function generateContent(topic, outline, cardsText) {
...
function offlineContent(topic, outline, cardsText) {
    const lines = cardsText.split("\n").filter(Boolean);
...
export async function runArticleJob(jobId, deckId, topic) {
    const emit = (payload) => articleEvents.emit(jobId, payload);
