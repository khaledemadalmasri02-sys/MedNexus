...
export class PDFService {
  ...
  async extractText(
    ...
    try {
      ...
      const pagePromises = Array.from({ length: pageCount }, (_, i) => i + 1).map(
        async (pageNum): Promise<PDFPage> => {
  ...
  async extractPages(
    ...
    try {
      ...
      throw new Error(`Failed to extract pages from PDF: ${(error as Error).message}`);
    }
  }
