...
class LRUCache {
  ...
  constructor(maxSize: number, ttlMs: number = 3600000) { this.maxSize = maxSize; this.ttlMs = ttlMs; }
  get(key: string): CacheEntry | undefined {
    const entry = this.map.get(key);
    if (!entry) return undefined;
    const ts = this.timestamps.get(key) || 0;
    if (Date.now() - ts > this.ttlMs) { this.map.delete(key); this.timestamps.delete(key); return undefined; }
    this.map.delete(key); this.map.set(key, entry); this.timestamps.set(key, Date.now());
    return entry;
  }
  set(key: string, entry: CacheEntry) {
    if (this.map.size >= this.maxSize) { const firstKey = this.map.keys().next().value; if (firstKey) { this.map.delete(firstKey); this.timestamps.delete(firstKey); } }
    this.map.set(key, entry); this.timestamps.set(key, Date.now());
  }
...
function normalize(q: string): string { return q.toLowerCase().trim().replace(/[?!.,;:'"]/g, "").replace(/\s+/g, " "); }
function hash(q: string): string { return crypto.createHash("sha256").update(normalize(q)).digest("hex"); }
function tokenize(q: string): string[] { return normalize(q).split(/\s+/).filter(w => w.length > 2); }

export async function getCachedResponse(agentId: string, question: string): Promise<CacheEntry | null> {
  const hashKey = hash(question);
  const now = Date.now();

  const memEntry = memoryCache.get(hashKey);
  if (memEntry) { await trackAnalytics(agentId, "memory_hit"); return memEntry; }

  const knowledgeHit = await searchKnowledgeBase(agentId, question);
  if (knowledgeHit) {
    const entry: CacheEntry = { answer: knowledgeHit.answer, source: "knowledge", confidence: 0.95 };
  ...
  if (dbEntry) {
    const entry: CacheEntry = { answer: dbEntry.answer, source: "ai", confidence: dbEntry.confidence };
    memoryCache.set(hashKey, entry);
...
export async function storeCachedResponse(agentId: string, question: string, answer: string, source: "ai" | "knowledge" = "ai", confidence: number = 0.8): Promise<void> {
  const hashKey = hash(question);
  const now = new Date();
...
async function searchKnowledgeBase(agentId: string, question: string): Promise<{ answer: string } | null> {
  ...
  for (const entry of entries) {
    let score = 0;
    const entryKeywords = JSON.parse(entry.keywords as string) as string[];
    for (const kw of keywords) { for (const ekw of entryKeywords) { if (ekw.includes(kw) || kw.includes(ekw)) score += 5; } }
...
async function trackAnalytics(agentId: string, type: string) {
  ...
  try {
    ...
    if (existing) {
      const u: Record<string, unknown> = { totalQuestions: sql`${agentCacheAnalytics.totalQuestions} + 1` };
  ...
  } catch (err) { logger.error({ err }, "Cache analytics error"); }
}

export async function getCacheStats(agentId: string) {
  ...
  const total = await db.select({ count: sql<number>`count(*)` }).from(agentResponseCache).where(eq(agentResponseCache.agentId, agentId));
...
export async function searchKnowledge(agentId: string, query: string, limit: number = 5) {
