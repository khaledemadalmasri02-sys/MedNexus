...
function generateQuestionFromSentence(sentence: string, keyTerms: string[]): GeneratedCard | null {
  ...
  const termToBlank = keyTerms.find(term => 
    sentence.toLowerCase().includes(term.toLowerCase()) && term.length > 3
  );
  ...
  const question = sentence.replace(
    new RegExp(`\\b${escapeRegex(termToBlank)}\\b`, "i"),
    "___________"
  );
...
function generateDefinitionCard(term: string, context: string): GeneratedCard | null {
  ...
  const definitionSentence = sentences.find(s => 
    s.toLowerCase().includes(term.toLowerCase()) && s.length > 30
  );
...
export class OfflineGenerator {
   ...
   */
  generateCards(text: string, count: number = 10): GeneratedCard[] {
   ...
   */
  generateQuestions(text: string, count: number = 10): GeneratedQuestion[] {
   ...
   */
  async *streamGenerateCards(text: string, count: number = 10): AsyncGenerator<{ type: string; data: unknown }> {
   ...
   */
  async *streamGenerateQuestions(text: string, count: number = 10): AsyncGenerator<{ type: string; data: unknown }> {
