...
// In-memory queue for offline items (in production, use a proper queue like Bull/BullMQ)
...
router.get("/queue", async (req: Request, res: Response) => {
  try {
    ...
    let items = Array.from(offlineQueue.values()).filter(
...
router.post("/process", async (req: Request, res: Response) => {
  try {
    const pendingItems = Array.from(offlineQueue.values()).filter(
