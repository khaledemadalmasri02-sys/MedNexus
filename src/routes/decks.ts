...
import {
  createDeckSchema, updateDeckSchema, mergeDecksSchema, idParamSchema,
} from "./validators.js";
import { db, decks, cards, mindMaps, freeTierUsage, qbanks, questions } from "../db/index.js";
...
async function checkDeckQuota(identifier: string): Promise<{ allowed: boolean; count: number }> {
...
async function recordDeckCreation(identifier: string): Promise<void> {
...
// Create new deck
router.post("/decks", validateBody(createDeckSchema), async (req: Request, res: Response) => {
...
router.get("/decks/tree", async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
...
// Update deck
router.patch("/decks/:id", validateParams(idParamSchema), validateBody(updateDeckSchema), async (req: Request, res: Response) => {
...
// Delete deck
router.delete("/decks/:id", validateParams(idParamSchema), async (req: Request, res: Response) => {
  const id = req.params.id as unknown as number;
...
// Merge decks
router.post("/decks/merge", validateBody(mergeDecksSchema), async (req: Request, res: Response) => {
...
// ── Move deck to folder ──
router.post("/decks/:id/move", validateParams(idParamSchema), async (req: Request, res: Response) => {
  const id = req.params.id as unknown as number;
...
// ── Get qbank tree (nested JSON) ──
interface QBankTreeNode {
  id: number;
  name: string;
  parentId: number | null;
...
router.get("/qbanks/tree", async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    ...
    const buildTree = (parentId: number | null): QBankTreeNode[] => {
      return allQbanks
