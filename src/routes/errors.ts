...
router.get("/common", async (req: Request, res: Response) => {
  try {
    const model = req.query.model as string | undefined;
    const minOccurrences = parseInt(req.query.min_occurrences as string) || 2;

    const patterns = await errorLearningService.getCommonPatterns(
      model,
      minOccurrences
    );
...
router.get("/similar", async (req: Request, res: Response) => {
  try {
    ...
    const errors = await errorLearningService.findSimilarErrors(
      operation as string,
      model as string,
      limit
    );
...
router.post("/:id/resolve", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const success = await errorLearningService.resolveError(
      errorId,
      resolution_notes,
      fix_pattern
    );
...
router.post("/resolve", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const count = await errorLearningService.bulkResolveByType(
      error_type,
      model,
      resolution_notes,
      fix_pattern
