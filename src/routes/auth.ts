...
import {
  ...
  getProgressiveLockout,
  recordProgressiveFailure,
  clearProgressiveLockout,
...
function getSessionTtl(rememberMe?: boolean): number {
...
function checkLoginLockout(req: Request, res: Response): boolean {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (checkLoginLockout(req, res)) return;

    const user = await findUserByEmail(email);

    if (!user || !user.passwordHash) {
      recordProgressiveFailure(`ip:${getClientIp(req)}`);
      if (email) recordProgressiveFailure(`email:${email}`);
      res.status(401).json({
    ...
    if (email) clearProgressiveLockout(`email:${email}`);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    let msUser: { id: string; mail?: string; userPrincipalName?: string; displayName?: string } | null = null;
