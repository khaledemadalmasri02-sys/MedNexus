...
import { osceAnalyticsRoutes } from "./routes/osce-analytics";
...
app.use("/api/admin/*", async (c, next) => {
  ...
  const allowedIp = c.env.ADMIN_ALLOWED_IP;
  if (allowedIp && clientIp !== allowedIp) {
    return c.json({ error: { code: "FORBIDDEN", message: "IP not allowed" } }, 403);
  }
  await next();
});
...
const apiRouters = [
  ...
  osceRoutes, osceAnalyticsRoutes,
];
